...
async function connectWeb3() {
  const provider = new Web3.providers.HttpProvider(
    "https://sepolia.infura.io/v3/195e71b77ab64dd7ab418e120b6b3154"
  );
  ...
  const instance = new web3.eth.Contract(
    VotingContract.abi,
    "0xE0769881D2b88e00209a72Dfc29839533bc25c36"
  );
...
async function registerCandidates(
  ...
  try {
    let res2 = await contractInstance.methods
      .registerCandidates(_name, Number(_age), _candidateAddress)
...
async function whiteListAddress(contractInstance, account, _voterAddress) {
  try {
    let res2 = await contractInstance.methods
      .whiteListAddress(_voterAddress)
      .send({ from: account });
    console.log("Res:", res2);
    return { error: false, message: res2.events.success.returnValues.msg };
  } catch (error) {
...
async function getWinner(contractInstance, account) {
  try {
    ...
    return {
      error: false,
      message: {
        candidateAddress: res2.candidateAddress,
        age: res2.age,
        name: res2.name,
      },
    };
